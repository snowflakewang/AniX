...
from ...modules import HYVideoARV2VDiffusionTransformer as HYVideoDiffusionTransformer # 250724
...
class HunyuanVideoCustomARV2VPipeline(DiffusionPipeline):
    r"""
    ...
    def encode_prompt(
        ...
        if prompt_embeds is None:
            ...
            if text_encoder.multi_view_ids is None: # make inference pipeline compatible with single/multi-view image condition
                if isinstance(self.args.multi_view_ids, int):
                    text_encoder.multi_view_ids = [self.args.multi_view_ids]
                elif isinstance(self.args.multi_view_ids, list):
                    text_encoder.multi_view_ids = self.args.multi_view_ids
                else:
                    raise Exception(f"Unsupported multi_view_ids type {type(self.args.multi_view_ids)}. 'int' and 'list' are supported.")
    ...
    def prepare_latents(self, batch_size, num_channels_latents, num_inference_steps,height, width, frame, dtype, device, timesteps,generator, latents=None, denoise_strength=1.0, pre_frame_latents=None):
        shape = (
            batch_size,
        ...
        # 250625 conditional input video latent frames
        if pre_frame_latents is not None and hasattr(self.args, "num_clean_latent_frames") and self.args.num_clean_latent_frames > 0:
            # assume input video is a 129-frame one generated by HunyuanVideo
            # maybe modified in the future
            # assert pre_frame_latents.shape == latents.shape
            
            # latents = torch.cat(
            #     [
            #         pre_frame_latents[:, :, :self.args.num_clean_latent_frames, :, :],
            #         latents[:, :, self.args.num_clean_latent_frames:, :, :]
            #     ],
            #     dim=2,
            # )
            # print(f"replace first {self.args.num_clean_latent_frames} latent frames with clean conditional video first latents.")
            latents = torch.cat(
                [
                    pre_frame_latents[:, :, -self.args.num_clean_latent_frames:, :, :],
                    latents[:, :, self.args.num_clean_latent_frames:, :, :]
            ...
            print(f"replace first {self.args.num_clean_latent_frames} latent frames with clean conditional video last latents.")
    ...
    def __call__(
        ...
        bg_latents: Optional[torch.Tensor] = None, # 250724
        audio_prompts: Optional[torch.Tensor] = None, # 250724
        pixel_value_llava: Optional[torch.Tensor] = None,
        uncond_pixel_value_llava: Optional[torch.Tensor] = None,
        pre_frame_latents: Optional[torch.Tensor] = None,
        ...
        **kwargs,
        ...
        self.check_inputs(
            ...
            audio_prompts, # 250724
            negative_prompt, # 250724
            prompt_embeds,
        ...
        if self.do_classifier_free_guidance:
            ...
            # 250724, copy from HYCustom-V2V/hymm_sp/diffusion/pipelines/pipeline_hunyuan_video_custom.py
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if pre_frame_latents is not None and self.args.num_clean_latent_frames > 0 and self.args.use_diffusion_forcing:
                    # 250704, use diffusion forcing when inference, we need to set different timesteps to video chunks
                    stabilization_level = 15
                    t = t.unsqueeze(0) # torch.Size([]) -> torch.Size([1])
                    t_ctx = torch.full(t.shape, stabilization_level - 1, device=t.device, dtype=t.dtype)
                    t = torch.cat([t_ctx, t], dim=0).unsqueeze(0) # [2] -> [1, 2]
                    t_expand = t.repeat(latent_model_input.shape[0], 1)

                    latent_frame_indices = torch.tensor([self.args.num_clean_latent_frames]).to(t.device)

                elif pre_frame_latents is not None and self.args.num_clean_latent_frames > 0:
                    # 250704, noise augmentation, the whole sequence shares the same timestep
                    t_expand = t.repeat(latent_model_input.shape[0])
                    latent_frame_indices = None
                
                elif self.args.num_clean_latent_frames > 0:
                    # 250807, noise augmentation, the whole sequence shares the same timestep
                    # but no zero padding to latents, so pre_frame_latents is None
                    t_expand = t.repeat(latent_model_input.shape[0])
                    latent_frame_indices = None
                ...
                with torch.autocast(device_type="cuda", dtype=target_dtype, enabled=autocast_enabled):
                    ...
                    if latent_model_input.shape[-1]*latent_model_input.shape[-2]>64*112 and cpu_offload:
                        ...
                        noise_pred = self.transformer(             # For an input image (1, 256, 256)
                            ...
                            bg_latents=bg_latents, # 250724, copy from HYCustom-V2V
                            audio_prompts=audio_prompts, # 250724, copy from HYCustom-V2V
                            audio_strength=audio_strength, # 250724, copy from HYCustom-V2V
                            ...
                            return_dict=True,
                            is_cache=is_cache,
                            latent_frame_indices=latent_frame_indices, # 250704
                        )['x']
                ...
                # 250625 replace first num_clean_latent_frames latents to guidance the next iteration of transformer denoising
                if pre_frame_latents is not None and hasattr(self.args, "num_clean_latent_frames") and self.args.num_clean_latent_frames > 0:
                    # latents = torch.cat(
                    #     [
                    #         pre_frame_latents[:, :, :self.args.num_clean_latent_frames, :, :],
                    ...
                    # print(f"replace first {self.args.num_clean_latent_frames} latent frames with clean conditional video latents after a denoising iteration.")
