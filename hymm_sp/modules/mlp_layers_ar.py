...
class FinalLayer(nn.Module):
    ...
    def forward(self, x, c, latent_frame_indices=None):
        shift, scale = self.adaLN_modulation(c).chunk(2, dim=1)
        # 250704
        if latent_frame_indices is not None: # not None means use diffusion forcing
            shift = shift.reshape((-1, 2, shift.shape[-1])).mean(dim=1, keepdim=False)
            scale = scale.reshape((-1, 2, scale.shape[-1])).mean(dim=1, keepdim=False)
