...
class IndividualTokenRefinerBlock(nn.Module):
    ...
    def forward(
        ...
        # 250703 if different timesteps for different video chunks
        # context_aware_representations.shape=[1, 3072], timestep_aware_representations.shape=[input_t.shape[0], 3072]
        if latent_frame_indices is not None:
            # 250704
            # hard-code now, for diffusion forcing, one seq needs 2 timesteps, one for previous chunk, one for next chunk
            gate_msa = gate_msa.reshape((-1, 2, gate_msa.shape[-1])).mean(dim=1, keepdim=False)
            gate_mlp = gate_mlp.reshape((-1, 2, gate_mlp.shape[-1])).mean(dim=1, keepdim=False)
...
class IndividualTokenRefiner(nn.Module):
    ...
    def forward(
        ...
        latent_frame_indices: torch.LongTensor=None,
    ):
        self_attn_mask = None
        if mask is not None:
        ...
        for block in self.blocks:
            ...
            x = block(x, c, self_attn_mask, latent_frame_indices) # 250704
        return x
...
class SingleTokenRefiner(nn.Module):
    ...
    def forward(
        ...
        if latent_frame_indices is not None:
            assert t.ndim == 2
            timestep_aware_representations = self.t_embedder(t.reshape(-1))
        else:
            timestep_aware_representations = self.t_embedder(t)
        ...
        # 250704
        if latent_frame_indices is not None:
            c = timestep_aware_representations + context_aware_representations.repeat_interleave(2, dim=0)
        else:
            c = timestep_aware_representations + context_aware_representations
        ...
        x = self.individual_token_refiner(x, c, mask, latent_frame_indices)
