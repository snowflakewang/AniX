...
def get_nd_rotary_pos_embed_new(rope_dim_list, start, *args, theta=10000., use_real=False, 
    ...
    if len(concat_dict)<1:
        ...
        if concat_dict['mode']=='timecat':
            ...
            # #grid = torch.cat([bias, grid], dim=1)
            # if 'mv_encode_mode' in concat_dict.keys() and concat_dict['mv_encode_mode'] == 'separate':
            #     grid = torch.cat([bias, bias, bias, bias, grid], dim=1) # only compatible with separately VAE encoding 4-view
            # else:
            #     grid = torch.cat([bias, grid], dim=1)
            if isinstance(concat_dict['bias'], int):
                bias = grid[:,:1].clone()
                bias[0] = concat_dict['bias']*torch.ones_like(bias[0])
                bias[2] += start[-1]
                grid = torch.cat([bias, grid], dim=1)
            elif isinstance(concat_dict['bias'], list):
                for bias_value in concat_dict['bias']:
                    tmp_bias = grid[:,:1].clone()
                    tmp_bias[0] = bias_value*torch.ones_like(tmp_bias[0])
                    tmp_bias[2] += start[-1]
                    try:
                        bias = torch.cat([bias, tmp_bias], dim=1)
                    except:
                        bias = tmp_bias
                grid = torch.cat([bias, grid], dim=1)
