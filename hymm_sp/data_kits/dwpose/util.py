...
def draw_bodypose(canvas, candidate, subset, score):
    ...
    for i in range(18):
        for n in range(len(subset)):
            ...
            x = int(x * W)
            y = int(y * H)
            cv2.circle(canvas, (int(x), int(y)), 1, alpha_blend_color(colors[i], conf), thickness=-1)
...
def draw_handpose(canvas, all_hand_peaks, all_hand_scores):
    ...
    for peaks, scores in zip(all_hand_peaks, all_hand_scores):
        ...
        for i, keyponit in enumerate(peaks):
            ...
            if x > eps and y > eps:
                cv2.circle(canvas, (x, y), 1, (0, 0, score), thickness=-1)
    return canvas
...
def draw_facepose(canvas, all_lmks, all_scores):
    ...
    for lmks, scores in zip(all_lmks, all_scores):
        for lmk, score in zip(lmks, scores):
            ...
            conf = int(score * 255)
            if x > eps and y > eps:
                cv2.circle(canvas, (x, y), 1, (conf, conf, conf), thickness=-1)
    return canvas

def draw_pose(pose, H, W, use_hand=True, use_body=True, use_face=True, ref_w=2160):
    bodies = pose['bodies']
    faces = pose['faces']
    hands = pose['hands']
